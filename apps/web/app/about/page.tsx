...
const STAGES = [
  ...
  ["04", "Reverse image search", "A real search provider returns real candidates. With no API key configured, RAYA reports the search as unavailable; it never substitutes placeholder results."],
  ["05", "Candidate filtering", "Results are classified by platform. Only known public social sources become candidates, and everything filtered out is still counted and shown."],
  ["06", "Independent verification", "RAYA re-downloads each candidate image itself, detects a face with its own detector, encodes it with its own encoder, and compares. The search engine does not get a vote."],
  ["07", "Evidence", "A canonical JSON record is built and hashed. Canonical means byte-reproducible: the same logical record always yields the same digest."],
  ["08", "IPFS", "The bundle is stored under a content address, so the CID cannot later point at different content."],
  ["09", "Anchor", "The evidence hash, input hash, source hash and CID are written to a write-once contract on Core Testnet2."],
  ["10", "Read-back", "After confirmation, RAYA makes a fresh call to the contract and compares the stored hash with the local one."],
  ["11", "Integrity", "Only if that comparison passes does RAYA say the evidence is intact. A submitted transaction is not treated as success."],
];
...
const LIMITS = [
  ...
  ["The threshold is a policy choice.", "0.40 trades recall for precision against recompressed web images. Lowering it finds more true matches and more false ones. It is recorded in every evidence bundle so a reader knows what was applied."],
      ...
      <div style={{ paddingTop: "var(--s7)", maxWidth: 760 }}>
        ...
        <section style={{ marginTop: "var(--s7)" }}>
          ...
          <p className="body" style={{ marginTop: "var(--s4)", fontSize: 13.5 }}>
            Face detection and encoding run locally via OpenCV. No face image or
            embedding is sent to a third-party face API, written into the
            evidence bundle, uploaded to IPFS, or anchored on chain. What is
            published is hashes, public URLs, a similarity score and the names of
            the models used.
          </p>
          <p className="body" style={{ marginTop: "var(--s3)", fontSize: 13.5 }}>
            One consequence is worth stating directly: to run a reverse image
            search at all, the input image must be reachable by the search
            provider. RAYA does this by publishing the input to IPFS and handing
            the provider that URL. If you do not want an image published, do not
            run a search on it.
          </p>
        </section>
